...
- Types the engine reads only through a hand-written deserializer are now modeled mechanically from that deserializer's key reads, so future game updates pick them up without curation. This also covers the `Dashes` entries of a GUI circle renderer (`DashURange`, cap lengths and ranges).
...
- Reference-path completion now completes the segment at the cursor rather than the whole written value, so editing a middle segment of a long `&<…>/Part/Components/…` path offers that segment's members instead of repeatedly suggesting a match for the final segment.
- Reference-path completion now matches member names case-insensitively like the game, so a lower-case prefix offers the capitalized member and a mid-path segment written in the wrong case still walks into the right container, matching how go-to-definition and validation already resolve it.
- References to a member a mod's action merges into a game node — a nested `Overrides`, or an `Add` with a `Name` (`AddTo=<…>/Part/Components`, `Name=…`) — now resolve everywhere: go-to-definition jumps to the action that declares it, completion offers it alongside the node's own members, and validation no longer reports it as unresolved.
...
- Completion now works inside cross-mod workshop references: after `&<./Data/../../../workshop/…/file.rules>/` the target file's members are offered and nested paths drill in. The file part of the escape kept its `>` when touching the disk, so the lookup always failed and the popup stayed empty.
