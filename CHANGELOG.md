...
- A hover over a modifiable value now lists what its modifiers do to the number. Each row says what drives the modifier, the mode it combines with, whatever it writes, and the clamp it puts on the result, and a modifier driven by a buff also names the parts that supply it, which nothing in the editor could answer before. A value the game only knows at runtime is never given a made-up number: what is shown is the interval the file proves. A group that writes both a `Modifiers` list and a `BuffType` shortcut is told that the shortcut is ignored, which is the rule the game reads it by and is invisible in the file. Turn it off with `cosmoteerLSPRules.hover.showModifiers`.
