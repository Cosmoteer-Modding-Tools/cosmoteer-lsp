...
/**
 * Regression: a parenthesized nested function call used as a math operand —
 * `ceil((ceil(&b))/4)` — used to leak the `(` it opened (the `startWithParens`
 * shortcut consumed it, then the nested-call guard skipped the matching close),
 * desyncing paren matching so EVERY sibling after the assignment was dropped from
 * the group. That silently truncated the AST (e.g. a part's `Components`),
 * producing false "reference name is not known" diagnostics for paths into the
 * dropped members. See the real-world `AtlasSprite = …/DamageLevels/0` case.
 */
 ...
 * fields and swallowed the following group's identifier (a SILENT desync: zero parser errors). Seen on
 * vanilla `ION_ENERGY = -(&Part/Components/…/BaseValue)` in ion_beam_emitter.rules, which corrupted the
 * `Part` group so every ref into it was flagged "reference name is not known".
 */
