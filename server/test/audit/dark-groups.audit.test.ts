...
import { expectedValueType, groupDiscriminator, resolveGroupClass } from '../../src/document/schema/schema-context';
import { classByDiscriminator, fieldOf } from '../../src/document/schema/schema';
...
// One-off audit harness (env-gated, never runs in CI): classify every group across vanilla and every
// installed workshop mod by whether resolveGroupClass finds a class, and when it does, whether the
// class actually fits the group's own field names. The output JSON drives the "why is this file dark"
// triage. Usage:
//   AUDIT_OUT=<report.json> npx vitest run test/audit/dark-groups.audit.test.ts
...
const HAVE = !!OUT && existsSync(DATA_DIR) && existsSync(MODS_DIR);
...
describe.skipIf(!HAVE)('dark-group audit over vanilla + workshop mods', () => {
    it('classifies every unresolved or badly-fitting group', async () => {
        ...
        const auditGroup = (group: GroupNode, doc: AbstractNodeDocument, file: string): void => {
            groupsTotal++;
            const cls = resolveGroupClass(group);
            const names = namedMemberNames(group);
            const name = group.identifier?.name ?? '(anonymous)';
            const line = group.position?.line ?? 0;
            if (cls) {
                groupsResolved++;
                // Fit check: a resolved class that owns under half of the group's own field names is
                // a mis-typing signal (the `Type = Beam` media-effect case).
                if (names.length >= 3) {
            ...
            if (parent && isDocumentNode(parent)) {
                const root = documentRootClass(parent);
                if (root) {
                    bump('root-class-misses-member');
                    findings.push({ file, line, group: name, cause: 'root-class-misses-member', detail: root });
                } else if (group.inheritance?.length) {
                    bump('unrooted-top-group-with-base');
                    findings.push({ file, line, group: name, cause: 'unrooted-top-group-with-base' });
                } else {
        ...
        const visit = (node: AbstractNode, doc: AbstractNodeDocument, file: string): void => {
            if (isGroupNode(node)) auditGroup(node, doc, file);
            const kids: AbstractNode[] =
                isGroupNode(node) || isListNode(node) || isDocumentNode(node)
                    ...
                    : isAssignmentNode(node)
                      ...
                      : [];
            for (const kid of kids) visit(kid, doc, file);
        };
        ...
        writeFileSync(OUT, JSON.stringify({ files: files.length, groupsTotal, groupsResolved, counts, findings }, null, 1));
        console.log('files', files.length, 'groups', groupsTotal, 'resolved', groupsResolved, JSON.stringify(counts));
        expect(groupsTotal).toBeGreaterThan(0);
    }, 1_200_000);
});
