...
import { clearNavigationMemo, navigate } from '../../../src/semantics/navigate-reference';
...
describe('super-path resolution across workspace init (startup race)', () => {
    it('does not pin a pre-init super-path miss that resolves once the game root is ready', async () => {
        ...
        const before = await navigate('&/Palette/Main', start, location, token);
        ...
        const after = await navigate('&/Palette/Main', start, location, token);
