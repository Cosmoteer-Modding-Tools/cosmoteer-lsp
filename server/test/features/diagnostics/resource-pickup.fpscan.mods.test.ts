...
// Triage scan of the resource-pickup check over every installed workshop mod, one mod at a time in
...
// The folders the pass reads the project through, set per mod as the scan walks them.
let scanFolders: string[] = [];

type Pass = (document: AbstractNodeDocument, token: CancellationToken) => Promise<ValidationError[]>;

const PASSES: { name: string; run: Pass }[] = [{ name: 'pickup', run: (document, token) => validateResourcePickups(document, scanFolders, token) }];
...
describe.skipIf(!HAVE)('the resource-pickup check over installed workshop mods', () => {
    it('collects every finding per mod for false-positive triage', async () => {
        ...
        console.log('[pickupscan] judged ' + JSON.stringify(Object.fromEntries(judged)));
