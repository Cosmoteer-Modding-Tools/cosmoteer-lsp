...
// vitest run mode drops large console output, so the scan also writes its findings to a report file
...
describe.skipIf(!HAVE_DATA)('path values over vanilla Data', () => {
    ...
    it('path values: zero findings', async () => {
        ...
        for (const file of filesUnder(DATA_DIR, '.rules')) {
            ...
            for (const error of await validatePathValues(doc, token)) {
