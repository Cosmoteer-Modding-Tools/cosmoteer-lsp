...
// Triage scan of the galaxy-generator checks over every installed workshop mod, one mod at a time in
...
const PASSES: { name: string; run: Pass }[] = [{ name: 'galaxygen', run: validateGalaxyGenerators }];
...
describe.skipIf(!HAVE)('the galaxy-generator checks over installed workshop mods', () => {
    it('collects every finding per mod for false-positive triage', async () => {
        ...
        console.log('[galaxyscan] judged ' + JSON.stringify(Object.fromEntries(judged)));
