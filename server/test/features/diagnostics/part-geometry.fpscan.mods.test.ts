...
// Triage scan of the part-geometry checks over every installed workshop mod, one mod at a time in
...
const PASSES: { name: string; run: Pass }[] = [{ name: 'partgeometry', run: validatePartGeometry }];
...
describe.skipIf(!HAVE)('the part-geometry checks over installed workshop mods', () => {
    it('collects every finding per mod for false-positive triage', async () => {
        ...
        console.log('[geoscan] judged ' + JSON.stringify(Object.fromEntries(judged)));
