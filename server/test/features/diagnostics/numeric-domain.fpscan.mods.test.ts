...
// Triage scan of the numeric-domain checks over every installed workshop mod, one mod at a time in
...
const PASSES: { name: string; run: Pass }[] = [{ name: 'numericdomain', run: validateNumericDomains }];
...
describe.skipIf(!HAVE)('the numeric-domain checks over installed workshop mods', () => {
    it('collects every finding per mod for false-positive triage', async () => {
        ...
        console.log('[numericscan] judged ' + JSON.stringify(Object.fromEntries(judged)));
