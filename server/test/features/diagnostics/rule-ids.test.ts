...
import { GAME_DATA_RULES, RULES, ruleById, ruleIdFor, UNTAGGED_RULE_ID } from '../../../src/features/diagnostics/rule-ids';
...
describe('the rule table', () => {
    ...
    it('carries one rule for every diagnostics setting the server has', () => {
        ...
        expect(
            ...
            'a diagnostics setting without a rule leaves its findings reported under no name. Add an entry to server/src/features/diagnostics/rule-ids.ts.'
        ).toEqual([]);
    });
    ...
    it('marks only the passes that need the game index as needing it', () => {
        expect(GAME_DATA_RULES.map((rule) => rule.id)).toEqual([
            ...
            // Reads the stack out of the resource file an id names, so it needs the game tree.
