...
// Triage scan of the storage-cycle check over every installed workshop mod, one mod at a time in
...
const PASSES: { name: string; run: Pass }[] = [{ name: 'storagecycle', run: validateStorageCycles }];
...
describe.skipIf(!HAVE)('the storage-cycle check over installed workshop mods', () => {
    it('collects every finding per mod for false-positive triage', async () => {
        ...
        console.log('[storagescan] judged ' + JSON.stringify(Object.fromEntries(judged)));
