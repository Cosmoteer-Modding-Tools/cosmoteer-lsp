...
function resetProjectIndexes(): void {
    for (const index of PROJECT_INDEXES) index.reset?.();
    clearFsCaches();
...
/**
 * Answers the capability handshake. What the client is able to do is read out of the
 * capabilities it sends, and what the server offers is declared back to it.
 *
 * @param params the client's capabilities and the initialization options it was started with.
 * @returns the capabilities the server advertises for this session.
 */
const handleInitialize = async (params: InitializeParams) => {
...
const loadGameTree = async (scopeUri: string): Promise<void> => {
    ...
    connection.window
        ...
        .then(() => {
            connection.sendRequest(COSMOTEER_METHOD.openSettings, {
...
const startUpServer = async (): Promise<void> => {
    ...
    if (hasDidChangeWatchedFilesCapability) {
        ...
        // Asset files are watched too: their existence is memoized (navigate-asset),
...
/**
 * Runs the startup once the client has confirmed initialization.
 *
 * @param _params the protocol's empty payload.
 */
const handleInitialized = async (_params: InitializedParams) => {
    try {
        await startUpServer();
    } catch (error) {
...
/**
 * Re-reads the settings and decides what has to be recomputed under them: the game install, the
 * code-mod schema, the derived caches and the workspace pass.
 *
 * @param change the pushed settings, which a client on the pull model leaves empty.
 */
const handleDidChangeConfiguration = async (change: DidChangeConfigurationParams) => {
    if (hasConfigurationCapability) {
...
/**
 * Registers the lifecycle handlers: the capability handshake, the rename repair, the startup that
 * scans the game tree and warms the project indexes, and the configuration change that decides what
 * has to be recomputed under the new settings.
 */
export function register(): void {
    connection.onInitialize(handleInitialize);
    connection.workspace.onWillRenameFiles(handleWillRenameFiles);
    connection.onInitialized(handleInitialized);
    connection.onDidChangeConfiguration(handleDidChangeConfiguration);
}
