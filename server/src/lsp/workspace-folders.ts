...
/**
 * The client's workspace folders as uris, taken from the shared cache so no feature pays a client
 * round-trip for them. Each call answers with a fresh array, so a caller that appends a root of its
 * own leaves the next caller's list untouched.
 *
 * @returns the folder uris, empty when the client has no workspace folders.
 */
export const workspaceFolderUris = async (): Promise<string[]> =>
    ((await getWorkspaceFoldersCached()) ?? []).map((folder) => folder.uri);

/**
 * The same folders as {@link workspaceFolderUris}, as on-disk paths, for the features that walk the
 * file system rather than address documents by uri.
 *
 * @returns the folder paths, empty when the client has no workspace folders.
 */
export const workspaceFolderPaths = async (): Promise<string[]> =>
...
export async function searchFolderUris(): Promise<string[]> {
    const uris = await workspaceFolderUris();
