...
export const ValidationForMath: Validation<MathExpressionNode> = {
    ...
    callback: async (node: MathExpressionNode) => {
        ...
        for (const child of node.elements) {
            ...
            if (
                lastNode &&
                isExpressionNode(lastNode) &&
                isExpressionNode(child)
            ) {
                return {
                    message: l10n.t(
                        'Between two expressions should be a Value'
                    ),
                    node: child,
                };
            }
            ...
            if (
                isExpressionNode(child) &&
                node.elements[node.elements.length - 1] === child
            ) {
                return {
                    message: l10n.t(
                        'Last element in MathExpression should be a Value'
                    ),
                    node: child,
                };
            }
