 ...
 * the shared {@link navigate}, so a game-root `<Data/…>` include and a mod-overlay path
...
export class ReverseIncludeIndex extends WatchedDocumentIndex implements AliasMemberSource {
     ...
     * lives in the vanilla tree, so it is resolved through the shared {@link navigate},
    ...
    private async resolveTargetFile(
        ...
        const resolved = await navigate(fileRef, referenceNode, sourceUri, cancellationToken).catch(() => null);
        if (!resolved) return undefined;
        if (isFile(resolved as unknown as FileTree)) {
