 ...
 */
const targetDirectory = (dirPart: string, uri: string): string => {
 ...
 */
export const completeAssetPath = async (args: {
    node: ValueNode;
    ...
    const resolvedType = assetType ?? (type === 'Sprite' || type === 'Sound' || type === 'Shader' ? type : undefined);
    ...
    const dir = targetDirectory(dirPart, uri);
    try {
