...
/** Matches a value whose last segment is still a bare relative reference. */
 ...
 * path for each base in the enclosing inheritance, so inherited members are discoverable from the
 * first keystroke. The caret paths come from the nearest enclosing container, since `&^` resolves
 ...
 */
export const referenceStartCompletions = (node: AbstractNode): string[] => {
...
/**
 * Path-completion for reference values (file, super, in-file alias). Lists the members of the
 * referenced entity, supporting cross-file navigation and mod overrides.
 *
 * @param args the node, whether it is an inheritance node, and the cancellation token.
 * @returns the completions offered for the reference.
 */
export const completeReference = async (args: {
    node: ValueNode;
 ...
 *
 * @param path the path text to complete.
 * @param node the node the path is resolved from.
 * @param cancellationToken stops the traversal.
 * @returns the completions offered for the path.
 */
export const completeRawPath = async (
    path: string,
    node: AbstractNode,
    cancellationToken: CancellationToken
): Promise<Completion[]> =>
...
const traversePath = async (
    ...
    // .NET path APIs (see navigateRules in navigate-reference.ts). Normalize to `/` so the
 ...
 * `stepIntoNode` (document/reference-resolver.ts), the same resolver navigation, hover and validation
 * use, so completion can never diverge from them on what a segment resolves to (a class of bug this
 * used to reimplement). Cross-file/reference dereferencing is delegated to the shared reference navigation,
...
const traverseReferencePath = async (
    ...
    // the reference navigation uses, so completion resolves every segment type (`..`, `~`, `^`, `:`,
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (derefToContinue && isReferenceValueNode(stepped)) {
            const target = await navigate(
                String(stepped.valueType.value),
                stepped,
                getStartOfAstNode(stepped).uri,
                cancellationToken
            ).catch(() => undefined);
            if (cancellationToken.isCancellationRequested) throw new CancellationError();
            stepped =
