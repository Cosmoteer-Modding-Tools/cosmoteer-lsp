...
// Written with `as const` so every id is its own literal type, which {@link RuleId} reads. The
// public {@link RULES} below hands the same table out under the widened element type its readers
// walk it with.
const RULE_TABLE = [
    {
        id: 'parse-error',
    ...
    },
    {
        ...
        title: 'Part geometry',
        description:
            'A part-grid value the part own size puts out of reach, such as a door location that is not a cell beside the part, a door presence toggle whose cell is inside the part, or a resource access point that lands outside it.',
        defaultLevel: 'hint',
        setting: 'validatePartGeometry',
        needsGameData: false,
    },
    {
        id: 'validateResourcePickups',
        title: 'Pickup bigger than the stack',
        description:
            'A storage that hands a crew more of a resource than a crew member can carry, where the game subtracts the whole amount and the crew keeps only a stack, destroying the difference.',
        defaultLevel: 'warning',
        setting: 'validateResourcePickups',
        needsGameData: true,
    },
    {
        id: 'validateNumericDomains',
        title: 'Numbers the reader refuses',
        description:
            'A number the class reading it cannot survive, such as a continuous beam with no hit interval, a converter quantity of zero, or nugget art sliced into more tiers than the stack holds.',
        defaultLevel: 'error',
        setting: 'validateNumericDomains',
        needsGameData: false,
    },
    {
        id: 'validateStorageCycles',
        title: 'Storage composition cycles',
        description:
            'A resource storage composed out of itself, directly or round a ring of other storages, which the game answers by asking the same question forever.',
        defaultLevel: 'error',
        setting: 'validateStorageCycles',
        needsGameData: false,
    },
    {
        id: 'validateGalaxyGenerators',
        title: 'Galaxy generators',
        description:
            'A galaxy generator that builds a broken map: a career one that gives its systems no sector type, a progression priority that reaches past its own list, a connection radius no pair of nodes falls inside, or a starting-node filter that keeps nothing.',
        defaultLevel: 'error',
...
/**
 * The identities a finding may be stamped with. Read by the whole-document pass table, so a pass
 * whose `code` names no rule fails to compile rather than reaching a report as an unnamed finding.
 */
export type RuleId = (typeof RULE_TABLE)[number]['id'];

export const RULES: readonly LintRule[] = RULE_TABLE;
