 ...
 */
const isGameRootPath = (path: string): boolean => /^\s*\.[\\/]/.test(path);
...
const PATH_TOKEN = /[A-Za-z0-9_.\-/\\ ]*\.[A-Za-z][A-Za-z0-9]{0,6}/g;

/** Whether a token ends in an extension the game loads as an asset. */
const looksLikeAssetPath = (token: string): boolean => {
 ...
 */
export const analyzeReferences = (raw: string, declaringDir: string, baseDir: string): ReferenceVerdict => {
    ...
    for (let i = 0; i < raw.length; i++) {
        ...
        if (!isGameRootPath(inner)) {
            const rebased = rebasePath(inner, declaringDir, baseDir);
    ...
    for (const match of raw.matchAll(PATH_TOKEN)) {
        ...
        const rebased = rebasePath(trimmed, declaringDir, baseDir);
        if (rebased === undefined) return refuse;
        if (rebased !== trimmed) {
