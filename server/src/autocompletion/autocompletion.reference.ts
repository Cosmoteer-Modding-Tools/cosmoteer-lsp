import { isArrayNode, isAssignmentNode, isObjectNode, isValueNode, ValueNode } from '../parser/ast';
...
export class AutoCompletionReference implements AutoCompletion<ValueNode> {
    public getCompletions(node: ValueNode): string[] {
        if (isValueNode(node) && node.valueType.type === 'Reference' && node.valueType.value.startsWith('&')) {
