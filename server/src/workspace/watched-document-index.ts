...
export abstract class WatchedDocumentIndex {
    ...
    private static async buildAll(
        ...
        if (cacheable) {
            ...
            if (projectLoaded) {
                await WatchedDocumentIndex.applyProjectStampDiff(pending, project!.stamps, liveFolderPaths, progress);
                liveFromCache = true;
            } else {
                ...
                await WatchedDocumentIndex.buildGameTree(pending, gameFolders, dataRoot!, indexAll, progress);
            }
        } else {
        ...
        if (cacheable && !liveFromCache && liveFolders.length > 0) {
            await WatchedDocumentIndex.saveConvergedProject(pending, dataRoot!, liveFolderPaths);
        }
    ...
    /**
     * Reconciles a loaded project cache against the workspace files on disk: a file whose stamp
     * moved is marked dirty and a file the cache knew but disk no longer has is removed, so the
     * loaded state converges without a walk.
     *
     * @param pending the indexes being built.
     * @param savedStamps the workspace-file stamps the cache was saved with.
     * @param liveFolderPaths the workspace folder paths to stat.
     * @param progress the indexing progress reporter.
     * @returns once every pending index is marked converged.
     */
    private static async applyProjectStampDiff(
        pending: WatchedDocumentIndex[],
        savedStamps: ProjectFileStamp[],
        liveFolderPaths: string[],
        progress: WorkDoneProgressReporter
    ): Promise<void> {
        const current = await statProjectFiles(liveFolderPaths);
        const savedByKey = new Map(savedStamps.map((stamp) => [normalizeUri(stamp[0]), stamp]));
    ...
    /**
     * Serves the game tree from its own cache, or walks it and saves the converged state.
     *
     * @param pending the indexes being built.
     * @param gameFolders the game `Data` folders to walk on a cache miss.
     * @param dataRoot the game `Data` root the cache is keyed by.
     * @param indexAll the shared disk walk of the run.
     * @param progress the indexing progress reporter.
     * @returns once the game tree's share of every pending index is in place.
     */
    private static async buildGameTree(
        pending: WatchedDocumentIndex[],
        gameFolders: string[],
        dataRoot: string,
        indexAll: (folders: string[], diskOnly: boolean) => Promise<void>,
        progress: WorkDoneProgressReporter
    ): Promise<void> {
    ...
    /**
     * Persists the converged combined state for the next start. A file currently open in the
     * editor is saved without its stamp: its indexed content may be the buffer, and a missing
     * stamp makes the next start re-ingest it from disk.
     *
     * @param pending the indexes that were built.
     * @param dataRoot the game `Data` root the cache is keyed by.
     * @param liveFolderPaths the workspace folder paths the cache covers.
     * @returns once the cache was written.
     */
    private static async saveConvergedProject(
        pending: WatchedDocumentIndex[],
        dataRoot: string,
        liveFolderPaths: string[]
    ): Promise<void> {
