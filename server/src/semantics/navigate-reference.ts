 ...
 * handling in {@link navigateReference}).
 */
...
export const navigate = async (
    ...
    const resolve = (): Promise<AbstractNode | null | FileWithPath> => {
        ...
        if (path.startsWith('&/') || path.startsWith('/')) {
            return navigateSuperPath(path, cancellationToken, visited, inheritanceVisited);
        }
        if (path.startsWith('&') && startNode.parent) {
            const scope = relativeReferenceScope(path, startNode);
            if (!scope) return Promise.resolve(null);
            return navigateReference(path.substring(1), scope, cancellationToken, visited, inheritanceVisited);
        }
        return navigateReference(path, startNode, cancellationToken, visited, inheritanceVisited);
    };
...
const navigateReference = async (
    ...
    const resolveReference: ResolveReferenceFn = async (p, n, location, token, inherited) =>
        (await navigate(p, n, location, token, visited, inherited)) ?? (await resolveThroughModContext(p, n, token));
    ...
    for (const substring of substrings) {
        ...
        if (!node) {
            ...
            if (
                ...
                node = navigateReferenceRecursive(substring, lastNode.parent as AbstractNode);
                continue;
            }
            if (!node) return null;
        }
...
const navigateRules = async (
    ...
    if (isDataRoot && pathes[2] !== '..') {
        const file = navigateCosmoteerRules(pathes.slice(2, lastWorkspacePathIndex + 1));
...
const navigateRulesByCurrentLocation = async (
    pathes: string[],
    currentLocation: string,
...
const navigateReferenceRecursive = (substring: string, node: AbstractNode, isInheritance = false) =>
    stepIntoNode(node, substring, isInheritance);

const navigateCosmoteerRules = (pathes: string[]) => {
    const file = CosmoteerWorkspaceService.instance.findFile(pathes) ?? null;
    return file;
};

const navigateSuperPath = async (
    path: string,
    cancellationToken: CancellationToken,
    visited: Set<AbstractNode> = new Set(),
    inheritanceVisited: Set<AbstractNode> = new Set()
