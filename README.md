...
| `diagnostics.validateResourcePickups` | on | Report a storage handing a crew more of a resource than one crew member can carry. The storage subtracts the whole amount from itself while the crew keeps only a stack, so the difference is destroyed. The stack is read from the resource file the id names, with a manifest replacement of that member applied |
| `diagnostics.validateNumericDomains` | on | Report a number the class reading it cannot survive: a continuous beam with no positive `HitInterval`, which freezes the game the first time it fires; an `InlineResourceConverter` quantity below one; a `ShipIconGlowShipScale` of zero; and nugget art sliced into more tiers than `MaxPerNugget` holds |
| `diagnostics.validateStorageCycles` | on | Report a resource storage composed out of itself, directly or round a ring. The game answers what such a storage holds by asking the same question forever, and the process disappears the first time the part is built |
| `diagnostics.validateGalaxyGenerators` | on | Report a `Spawners` list that builds a map the game cannot use: a career generator running no `RandomSectorTypes`, which silently puts every star system on the first sector type; a `ProgressionNodeTiers` priority reaching past the array its own list sizes; a `ConnectionRadius` at or below the node spacing, which leaves the galaxy with no FTL routes; and a `StartingNodePicker` window or faction candidate count that discards every node |
| `diagnostics.validatePartGeometry` | on | Fade a door, blocked travel cell or wall entry the part's own size puts out of the game's reach, with a remove quick fix, and report a `PhysicalRect` that leaves the part as an error. Warn about a storage's `PickUpLocation` or `DeliveryLocation` and a resource grid's `GridRect` that land on cells the part does not own, and about a resource sink on a part crew cannot walk through, each of which can end a resource search with an index error |
...
To make server starts fast, the language server persists its project indexes (schema ids, includes, localization keys, word index), the list of mod files that carry no mod actions so a start does not read them again, the alias walk from `cosmoteer.rules`, and, when whole-workspace validation is enabled, the per-file validation results between sessions. Reopening an unchanged mod restores everything, Problems panel included, in about a second.
...
-   `Cosmoteer: New Content File` creates a new part, resource, shot, media effect, title screen ship, roof decal folder, build toolbar category, part stat line, part toggle, buff or codex page and wires it into the game in one step: the file is written with its required fields, the id is derived, the name and description keys are added to every language file, and a part is registered in the ship class you pick. A shot and a media effect are created with the reference to paste, since nothing in the game registers those. A toolbar category, a stat line, a toggle and a buff are registered but show nothing until a part names them, so the summary says what the part has to write
-   `Cosmoteer: New Tech` takes one of your parts, a cost and the techs a player needs first, writes the tech with the part's own name, description, icon and toolbar group read by reference, and adds it to the game's tech tree. Until a tech names it, a part is buildable from the start of a career
...
-   `Cosmoteer: Diff This Group Against the Game's Version` answers the same question for one group, comparing it against the nearest base of it the game ships and listing only what the game loads differently. A member the group leaves to its base is left out, since that is not a difference
-   Two drawn views share one panel. `Cosmoteer: Show Resource Flow` draws which storage feeds which converter in a part, with a sentence on each box saying what that component does and a label on each arrow saying how much of which resource moves along it and how often. The ship outside the part is drawn too, since a resource the crew deliver has to come from somewhere. `Cosmoteer: Show Firing Chain` follows the triggers from component to component, with each wait that works out to one number
-   `Cosmoteer: Show Ship Blueprint` reads a `.ship.png` out of the low bits of its own picture and lists every part it places, with a count and whether the project declares that part. A blueprint naming a part your mod has renamed loads with holes in it and the game says nothing. The command asks which ship to read when the editor is not on one
-   `Cosmoteer: Add Ships to a Faction` (also on the explorer menu of a `.ship.png` or a folder of them, and in the JetBrains Tools and project menus) puts saved ships into a faction's spawn pool with nothing typed: the tier is the game's own arithmetic over the price of the parts, the doors and the crew, the difficulty is read against what the game's own ships of that tier spend on weapons and armor, and the role comes from what the ship carries, so an unarmed ship with storage is offered as a trade ship, one built around its bunks as a crew transport, one that cannot fly as a station and an armed one as a combat ship. Each ship is copied into `builtin_ships/<faction>/<role>/` and registered the way the game's own are, a civilian ship gets its trade route, the faction's file is added to the built-in ships from the manifest, and every suggestion can be adjusted before anything is written. Two more roles cover what the game's own tree has beside those: a wreck for the debris fields, and a starter ship the career mode offers when a new game begins, with its description key declared in the language files. A station gets its stasis icon drawn beside it, the white silhouette the map shows while it is out of sight, the way the game's own icon generator draws it. Storage pods, the loot the game drops beside wrecks, are a role too. A whole folder of saved ships goes in at once
-   `Cosmoteer: New Faction` (also offered while adding ships) asks the id, the name and the border colour on one form, then writes the faction and everything the career mode needs to give it territory: the registry entry with free player indexes, its share of the galaxy and the tier ranges of its systems, an FTL beacon that marks them on the map, and the name in every language file, each wired in from the manifest. The form also takes a PNG for the icon, a saved ship for the beacon, and writes a lore page for the codex with its texts as keys to fill. Left empty, the icon and the beacon ship are the game's own until you replace them
-   `Cosmoteer: New Nebula` builds a nebula on one of the game's own looks, inherited whole with its shaders, effects and sounds, with your three colours and your spawn settings, and writes the nebula, its career spawner entry, its creative-mode doodad and its texts, each wired in from the manifest
-   `Cosmoteer: New Galaxy Size` clones the game's standard map generator with your number of systems and offers the size when a new career or creative game begins
-   `Cosmoteer: New Asteroid Type` writes a mineable asteroid type in one go. You pick the resource its deposits yield, the game deposit whose textures and palette icons it borrows until you draw your own, whether it spawns as a common type in the belts and fields, as a rare find marked on the map, or inside a sun's damage zone, and which sizes it comes in. The wizard writes the deposit tiles, soft and, if you want, hard ones that need a mining laser, the asteroid recipe per size, the spawner entries with the game's own size weights and the soft-to-hard conversions, and adds the manifest actions that put them into the asteroid class, the doodad registry, the shared conversion list and the rarity list you chose. The names become keys in every language file the mod ships
-   `Cosmoteer: New Planet` builds a planet on one of the game's own, inherited whole with its style, sizes and orbits, gives it your name and optionally your sizes, and offers it to the career sectors as an inner planet, an outer planet or a moon, or to the creative palette only. It writes the doodad and its name key, and wires the doodad registry and the sector spawner from the manifest
-   `Cosmoteer: Resource in Trade` makes trade ships carry a resource and stations stock it, or buy it, with figures on the scale the game's own goods use. It picks from your mod's resources and from game resources the game does not trade, and writes only the two manifest actions the trade reads
-   `Cosmoteer: New…` is the one entry to all of it, from the palette, from a folder's context menu and from the editor: a picker of everything the wizards can create, which hands over to the right one with that folder as the mod to write into
-   Moving or renaming a `.rules` file rewrites the references that name it, and the references the moved file itself writes, so the file lands with its wiring intact
-   `Cosmoteer: Compare Parts in a Table` (also in the JetBrains Tools and editor menus) puts every part of the game and of the mod being edited in a spreadsheet: one row per part, one column per field, every value resolved the way the game resolves it. A stat such as a shield's hit points is written in the game's own files as a division of two references living elsewhere in the file, and the table shows the figure that works out to. Columns are picked from everything the parts really carry, offered with the ones the parts differ in first, so a field a mod invented is as available as one the game ships. Rows narrow by part category, by component kind and by which mod they come from, and narrowing the rows narrows the columns with them: comparing the shields offers the two thousand fields shields carry rather than the twenty thousand the whole game does. Columns are dragged into the order you want them, sized by dragging their right edge, and frozen against the left edge from the arrow in the header. Three columns are worked out rather than read, since every hand-kept sheet has them: the cost in credits from the part's resources at the prices the game buys them for, the tiles the part covers, and the damage per second, taken from the game's own stat where the part declares one and multiplied by the barrels where a mod counts them. A tree at the left lists every ship class and, under it, the build menu groups the class's parts sit in, the way the game itself files them, and clicking a node narrows the table to it. Rows can be grouped the same way, by ship class and then build menu group, or by build menu group, category or mod alone, with each group folding away, a footer keeps the average, the least and the most of every column over the parts on screen, and a switch shows every number per tile. A formula column computes over the other columns with the functions of the rules math plus `if`, `coalesce` and `has` for a value a part may not have, a `ref` for the compared part, `colavg`, `colmin`, `colmax`, `colsum`, `colcount`, `colmedian` and `rank` over the parts on screen, a `*` in a path for every column it fits, and the name of another formula to build on it. The table follows the editor: an edit to a part, saved or not, reaches the table in the time that one part takes to read rather than the whole project. A cell can be double-clicked to try a value, the formulas and the comparison follow it while the files stay as they were, and writing the changes puts each typed value into its file, over the value the part writes itself or as an override on the part where the value came from a base. Picking a part to compare against shades every number against it, blue below that part and red above it, with a key beside the picker, any value can be clicked to open the declaration it was read from, and the whole table exports as an Excel workbook: a real table with its filter, the grouping as columns of its own, an average under every column, the comparison shading carried over, each part linking back to the file it is written in, and every formula column written out as an Excel formula the workbook computes itself. A whole setup, filters and columns and formulas and all, can be saved under a name, and the one you were last working in is put back the next time you open the table
-   Localization-key completion and hover for `KeyString` fields, with an insert-into-all-strings quick fix
-   The text markup a language file's strings carry is first class. Typing `<` inside a string offers every tag the game draws, closes the element for you and goes on to its attributes and the values they take, a `<string id=…>` offers the project's localization keys and an `<img name=…>` the images the project registers (the game's own text sprites plus one per resource and per faction), and every tag is checked against the element the game would run for it: one it draws nothing for, an attribute it throws without, a value it cannot parse and an image name the project registers nothing for all make the game draw the whole string with its tags as plain text
-   Color swatches with an in-place picker, in the `{ Rf Gf Bf }` groups of a `.rules` file and in the `<color r='250' g='176' b='86'>` tags of a language file alike, part-category completion
