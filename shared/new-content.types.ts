/**
 * The shapes the new-content command speaks in: what a client sends, the kinds it may create, where
 * each of them is wired in, and what creating one did. Read by the server that writes the file and by
 * every client that asks for one, so both sides share the one declaration and a kind or a reason added
 * here is a compile error in the client that has no wording for it yet.
 */

import { RegisterPartFailure } from './register-part.types';
