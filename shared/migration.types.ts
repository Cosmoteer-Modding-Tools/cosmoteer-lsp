/**
 * The shapes the migration commands answer a client in: what the bulk deprecation fix is invoked
 * with, and the summary both commands report, preview included. Read by the server that runs the
 * migration and by every client that shows what it did, so both sides share the one declaration.
 */
